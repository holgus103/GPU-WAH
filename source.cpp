...
int main(){
 ...
 // write headers
	...
	fs << "Density, ";
	fs << "Compressed size (data only) [Int] , ";
	...
	for (int s=1;s<=256;s<<=1) {
		for (int i=1;i<=16;i++) {
			...
			for(int repeat = 0; repeat < repetition; repeat++){
				...
				additional = sizeof(unsigned long long int) * blockCount;
				additional += sizeof(unsigned long long int) * blockCount;
				additional += sizeof(unsigned long long int);
...
//				free(compressed);
			...
			fs << globalCompressedSize + additional << ", ";
